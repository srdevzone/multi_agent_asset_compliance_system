...
class TestPutAuditRun:
    ...
    async def test_creates_record_in_progress(self, dynamodb_client):
        """put_audit_run should create a new IN_PROGRESS record."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-001", "asset-abc")
    ...
    async def test_raises_on_duplicate_run_id(self, dynamodb_client):
...
class TestGetAuditRun:
    ...
    async def test_returns_none_for_unknown_run(self, dynamodb_client):
        """get_audit_run should return None for a non-existent run_id."""
        result = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "not-exist")
        assert result is None

    async def test_returns_dict_for_existing_run(self, dynamodb_client):
        """get_audit_run should return a dict with status and asset_id."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-get", "asset-xyz")
        result = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-get")
    ...
    async def test_returns_verdict_when_complete(self, dynamodb_client):
        """get_audit_run should include the verdict dict when status is COMPLETE."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-done", "asset-xyz")
        verdict = {"compliance_status": "COMPLIANT", "confidence": 0.95}
        await dynamodb_service.complete_audit_run(dynamodb_client, TABLE_NAME, "run-done", verdict)

        result = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-done")
        assert result is not None
...
class TestCompleteAuditRun:
    ...
    async def test_sets_status_to_complete(self, dynamodb_client):
        """complete_audit_run should update status to COMPLETE and store verdict."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-c", "asset-abc")
        verdict = {"compliance_status": "NON_COMPLIANT", "confidence": 0.87}
        await dynamodb_service.complete_audit_run(dynamodb_client, TABLE_NAME, "run-c", verdict)

        result = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-c")
        assert result["status"] == dynamodb_service.STATUS_COMPLETE
...
class TestFailAuditRun:
    ...
    async def test_sets_status_to_failed(self, dynamodb_client):
        """fail_audit_run should update status to FAILED."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-f", "asset-abc")
        await dynamodb_service.fail_audit_run(dynamodb_client, TABLE_NAME, "run-f", "LLM timeout")

        result = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-f")
...
class TestEraseAssetRuns:
    ...
    async def test_erases_all_runs_for_asset(self, dynamodb_client):
        """erase_asset_runs should mark all runs for an asset as ERASED."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-e1", "erase-asset")
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-e2", "erase-asset")
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-e3", "other-asset")

        count = await dynamodb_service.erase_asset_runs(dynamodb_client, TABLE_NAME, "erase-asset")

        assert count == 2

        r1 = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-e1")
        r2 = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-e2")
        r3 = await dynamodb_service.get_audit_run(dynamodb_client, TABLE_NAME, "run-e3")
    ...
    async def test_returns_zero_for_no_runs(self, dynamodb_client):
        """erase_asset_runs should return 0 if no records exist for the asset."""
        count = await dynamodb_service.erase_asset_runs(dynamodb_client, TABLE_NAME, "ghost-asset")
        assert count == 0
...
class TestGetAssetRunSummary:
    ...
    async def test_returns_status_counts(self, dynamodb_client):
        """get_asset_run_summary should count runs by status."""
        await dynamodb_service.put_audit_run(dynamodb_client, TABLE_NAME, "run-s1", "sum-asset")
