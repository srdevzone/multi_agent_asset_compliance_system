...
async def test_document_agent_populates_state(
    mock_pinecone_index, mock_embeddings_model, sample_pinecone_match, make_audit_state
):
    ...
    with (
        ...
        result = await document_agent_node(make_audit_state(**_state_overrides()))
...
@pytest.mark.asyncio
async def test_document_agent_empty_namespace(
    mock_pinecone_index, mock_embeddings_model, make_audit_state
):
...
@pytest.mark.asyncio
async def test_document_agent_handles_pinecone_error(
    mock_pinecone_index, mock_embeddings_model, make_audit_state
):
...
@pytest.mark.asyncio
async def test_document_agent_handles_embedding_error(
    mock_pinecone_index, mock_embeddings_model, make_audit_state
):
