...
from app.schemas.image import ImageAnalysis


@pytest.mark.asyncio
async def test_image_agent_happy_path(mock_chat_model, s3_bucket, make_audit_state):
    ...
    with (
        ...
        result = await image_agent_node(make_audit_state(s3_image_keys=["audits/img001.jpg"]))
...
@pytest.mark.asyncio
async def test_image_agent_json_parse_error(mock_chat_model, s3_bucket, make_audit_state):
    ...
    with (
        ...
        result = await image_agent_node(make_audit_state(s3_image_keys=["audits/bad.jpg"]))
...
@pytest.mark.asyncio
async def test_image_agent_s3_download_error(mock_chat_model, s3_bucket, make_audit_state):
    ...
    with (
        ...
        result = await image_agent_node(make_audit_state(s3_image_keys=["missing/photo.jpg"]))
...
@pytest.mark.asyncio
async def test_image_agent_multiple_images_partial_failure(
    mock_chat_model, s3_bucket, make_audit_state
):
    ...
    with (
        ...
        result = await image_agent_node(
            make_audit_state(s3_image_keys=["audits/good.jpg", "audits/missing.jpg"])
        )
